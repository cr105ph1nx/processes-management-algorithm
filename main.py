...
from tkinter import *
from utils import fillQuantums, DICT_PROCESSES
...
def create_circles_frame(container):
    ...
    for process in DICT_PROCESSES:
        frame.create_oval(x0, y0, x1, y1, outline="black", fill=process,
                          width=0)
        x0 = x1 + 40
        x1 = x0 + (y1-y0)
...
def create_buttons_frame(container):
    frame = Frame(container)
    # Add ok button
    ok_button = Button(frame, text='Demarrer', width="30", height="2")
